...
exports.updateUser = async (req, res) => {
  ...
  if (req.file) {
    fields.push('avatar = ?');
    values.push(`/admin/media/avatars/${req.file.filename}`);
  }
...
exports.createUser = async (req, res) => {
  ...
  const id     = uuidv4();
  const avatar = req.file ? `/admin/media/avatars/${req.file.filename}` : null;
  ...
  await pool.execute(
    `INSERT INTO events
      (id, organizer_id, category_id, title, subtitle, description,
       cover_image, video_url, date, time, end_time, location, city, country,
       capacity, is_featured, tags, status)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      id, organizer_id, category_id || null,
      title, subtitle || null, description || null,
      cover_image, video_url, eventDate, eventTime, eventEndTime,
